       ...
       <command>
			...
			if( $doc->getElementsByTagName('status')) {
				if($doc->getElementsByTagName('status')->item(0)) {
					$statusres = $doc->getElementsByTagName('status')->item(0)->getAttribute('s');
                                        if($doc->getElementsByTagName('extension')->item(0)) {
                                          if ($doc->getElementsByTagName('rgpStatus')->item(0)){
                                           $rgpStatus= $doc->getElementsByTagName('rgpStatus')->item(0)->getAttribute('s');
					...
					continue;
                       ...
                       $queryresult = mysql_query("SELECT status FROM tbldomains WHERE registrar = 'COCCAepp' AND  domain='".$domain."'");
                       ...
                       $pendingStatus = 'Pending';
                        if (strcmp($domainStatus, 'pending') == 0) {
                         ...
                         elseif  ($resultpr == 'Object does not exist') {
                                ...
                                $sqlquery = "DELETE FROM tbldomains WHERE domain='".$domain."'";
                                  if ( mysql_query($sqlquery))  {
                                      echo ("|NOTICE: '".$domain."' Has been DELETED FROM WHMCS \n");
                                      echo ("|-----------------------------------------------------------\n");
                                     }
                                continue;
                               }
		...
		if ($statusres == "ok") {
			...
			mysql_query(sprintf($querytemplate,"Active",
                ...
                } elseif ($statusres == "pendingDelete" && $rgpStatus == 'redemptionPeriod') { 
                    echo "Domain : $domain has status :$rgpStatus  \n";
                   mysql_query(sprintf($querytemplate,"Redemption",
                                        mysql_real_escape_string($createdate),
                                        mysql_real_escape_string($nextduedate),
                                        mysql_real_escape_string($nextduedate),
                                        mysql_real_escape_string($domain)
